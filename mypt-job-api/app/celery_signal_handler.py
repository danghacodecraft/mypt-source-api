...
from app.http.serializers.queue_task_serializer import QueueTaskSerializer
...
def _task_sent_handler(sender=None, headers=None, body=None, **kwargs):
    try:
        ...
        if not serializer.is_valid():
            print(f"after_task_publish@validate:@{task_id}:", serializer.errors)
        else:
            serializer.save()
    except Exception as exc:
        print(f"after_task_publish@exc:{task_id}:", exc)
...
def _task_pre_execute(task_id, **kwargs):
    try:
        ...
        if not serializer.is_valid():
            print(f"task_prerun@validate@{task_id}:", serializer.errors)
        else:
            serializer.save()
        ...
        print(f"task_prerun@exc@{task_id}:", exc)
...
def _task_executed(task_id, retval, state, **kwargs):
    try:
        ...
        if not serializer.is_valid():
            print(f"task_postrun@validate@{task_id}:", serializer.errors)
        else:
            serializer.save()
            
    except Exception as exc:
        print(f"task_postrun@exc@{task_id}:", exc)
...
def _task_revoked(request, terminated, signum, expired, **kwargs):
    try:
        if terminated:
            ...
            if not serializer.is_valid():
                print(f"task_revoked@validate@{request.id}", serializer.errors)
            else:
                serializer.save()
    except Exception as exc:
        print(f"task_revoked@exc@{request.id}: ", exc)
