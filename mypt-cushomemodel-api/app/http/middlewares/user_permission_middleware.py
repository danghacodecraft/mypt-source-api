...
class UserPermissionMiddleware:
    ...
    def __call__(self, request):
        ...
        if curUrl in routesMiddlewareData:
            print(
                "[" + className + "] Da tim thay route : " + curUrl + " trong MIDDLEWARE_APPLIED_FOR_ROUTES. Duoc apply middleware nay !")
        ...
        if permissionsWithRoutesData.get(app_settings.SERVICE_NAME, None) is None:
            print(
                "[" + className + "] Ko co service " + app_settings.SERVICE_NAME + " trong redis permissionsWithRoutes ! Ko duoc di tiep API nay nua!")
        ...
        if servicePersRoutes.get(apiRoute, None) is None:
            print(
                "[" + className + "] Ko ton tai API route " + apiRoute + " trong service " + app_settings.SERVICE_NAME + " trong redis permissionsWithRoutes ! Ko duoc di tiep API nay nay!")
        ...
        print(
            "[" + className + "] API route " + apiRoute + " trong service " + app_settings.SERVICE_NAME + " duoc map voi quyen : ")
        print(perCodesByApiRoute)

        # check permisssions trong User session xem co quyen tinh nang nay ko
        userPerCode = ""
        userPerData = {}
        userPersData = request.user_auth_session_data["permissions"]
        ...
        if userPersData.get("ALL", None) is not None:
            ...
            for perCodeByApiRoute in perCodesByApiRoute:
                if userPersData.get(perCodeByApiRoute, None) is not None:
                    print(
                        "[" + className + "] User nay duoc xac nhan co quyen " + perCodeByApiRoute + "! Cho phep di tiep!")
                    userHasRight = True
        ...
        if userPerData["has_depart_right"] == 1:
            ...
            print("db name redis cua HO : " + str(project_settings.REDIS_HO_DATABASE_CENTRALIZED))
            ...
            for childDepartParam in childDepartsParamArr:
                ...
                for parentDepart, childDepartsByParent in userPerData["child_depart_rights"].items():
                    ...
                    if childDepartParam in childDepartsByParent:
                        print(
                            "[" + className + "] Da tim thay child depart " + childDepartParam + " trong parent depart " + parentDepart)
                ...
                if not hasRightOnChildDepart:
                    ...
                    if hasRightOnChildDepart == False and "ALL" in userPerData["branch_rights"]["PNC"]:
                        print(
                            "[" + className + "] hasRightOnChildDepart van bang false & co ALL trong branch_rights PNC")
                        for parent_depart, child_departs_by_parent in allDepartsWithLevels["allDeparts"][
                            "ALLPNC"].items():
                            print(
                                "[" + className + "] Tim trong parent depart cua branch_rights PNC : " + parent_depart)
                            if childDepartParam in child_departs_by_parent:
                                print(
                                    "[" + className + "] Da tim thay child depart " + childDepartParam + " trong parent depart cua branch_rights PNC " + parent_depart)
                                hasRightOnChildDepart = True
                                break
                    else:
                        print(
                ...
                if not hasRightOnChildDepart:
                    print(
                        "[" + className + "] Permission " + userPerCode + " cua user nay KHONG duoc phep apply cho child depart " + childDepartParam)
            ...
            if not hasRightOnChildDeparts:
                print(
                    "[" + className + "] Permission " + userPerCode + " cua user nay KHONG duoc phep apply tren 1 hoac vai child depart trong param childDepart! Nen KHONG DUOC PHEP di tiep API!")
