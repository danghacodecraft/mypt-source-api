...
def request_http(url, params=None, headers=None, get_request=False, auth=""):
    ...
    try:
        ...
        print("request_http >> Error/Loi {} >> {} ".format(url, e))
    return r
...
def call_api_get_start_checkin_date(emp_code, fname=""):
    date_join_company = ""
    url = PRIVATE_PROFILE_URL + NAME_SERVICE_PROFILE + "v1/get-info-emp-for-checkin"
    try:
        params = {
            "empCode": emp_code,
        }
        ...
        if res is not None:
            # print(res.status_code)
            data_api = res.json()
            data_output = data_api.get('data', {})
            date_join_company = data_output.get('dateJoinCompany', '')

            print("call_api_get_start_checkin_date {} - THANH CÔNG : res is NOT none \n \n".format(url))
        else:
            print("call_api_get_start_checkin_date >> {} >> THAT BAI : res is none ".format(url))
    except Exception as e:
        print("call_api_get_start_checkin_date {} >> THAT BAI VI LOI: {} \n \n".format(url, e))
