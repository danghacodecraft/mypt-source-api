...
class UserPermissionMiddleware:
    ...
    def __call__(self, request):
        ...
        if curUrl in routesMiddlewareData:
            # print("[" + className + "] Da tim thay route : " + curUrl + " trong MIDDLEWARE_APPLIED_FOR_ROUTES. Duoc
            # apply middleware nay !")
        ...
        if permissionsWithRoutesData.get(app_settings.SERVICE_NAME, None) is None:
            # print("[" + className + "] Ko co service " + app_settings.SERVICE_NAME + " trong redis
            # permissionsWithRoutes ! Ko duoc di tiep API nay nua!")
        ...
        if servicePersRoutes.get(apiRoute, None) is None:
            # print("[" + className + "] Ko ton tai API route " + apiRoute + " trong service " +
            # app_settings.SERVICE_NAME + " trong redis permissionsWithRoutes ! Ko duoc di tiep API nay nay!")
        ...
        # print("[" + className + "] API route " + apiRoute + " trong service " + app_settings.SERVICE_NAME + " duoc
        # map voi quyen : ") print(perCodesByApiRoute)
        ...
        if userPersData.get("ALL", None) is not None:
            ...
            for perCodeByApiRoute in perCodesByApiRoute:
                if userPersData.get(perCodeByApiRoute, None) is not None:
                    # print("[" + className + "] User nay duoc xac nhan co quyen " + perCodeByApiRoute + "! Cho phep
                    # di tiep!")
                    userHasRight = True
